...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        # I/O heads (linear projections, matching the first file's approach)
        ...
        # Two separate reasoning modules (H and L), each with their own weights
    ...
    def forward(
        ...
        # "Start" embeddings used only on the very first (H=0, L=0) step, matching first file
        input_embeddings_start = self._input_embeddings(
            torch.cat([
                batch["inputs"][:, :256],
                batch["inputs"][:, :256],
                batch["inputs"][:, 768:],
        ...
        # All but the final H+L step run without gradients
        with torch.no_grad():
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def forward(
        ...
        new_inner_carry, hidden_states, (q_halt_logits, q_continue_logits) = self.inner(new_inner_carry, new_current_data)

        outputs = {
            "hidden_states": hidden_states,  # continuous output, not logits
        ...
        with torch.no_grad():
            ...
            if self.training and (self.config.halt_max_steps > 1):
                ...
                # Target Q computed from the next inner step (no replay buffer / target network,
                # analogous to PQN https://arxiv.org/abs/2407.04811)
